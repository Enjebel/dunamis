      ...
      <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
            ...
            {posts.map((post) => (
              <div key={post._id} className="flex items-center justify-between gap-4 border border-slate-200 p-4">
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-slate-500">{post.category}{post.media?.length ? ` - ${post.media.length} media files` : post.mediaType && post.mediaType !== 'none' ? ` - ${post.mediaType}` : ''}</p>
                </div>
        ...
        <form id="post-form" onSubmit={submitPost} className="du-panel grid gap-4 p-6">
          ...
          <label className="du-field">
            <span className="du-label">Images or videos for the public website</span>
            <input className="du-input py-4" type="file" accept="image/*,video/*" multiple onChange={(e) => setMedia(Array.from(e.target.files || []))} />
            {media.length > 0 && <span className="text-xs font-bold text-slate-500">{media.length} file(s) selected</span>}
          </label>
